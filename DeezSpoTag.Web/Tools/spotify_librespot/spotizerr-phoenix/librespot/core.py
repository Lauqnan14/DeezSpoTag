...
class ApiClient(Closeable):
    ...
    def build_request(
        ...
        if suffix.startswith("/metadata/4/"):
            url = "https://spclient.wg.spotify.com" + suffix
        else:
            ...
            def anonymous():
                ...
                def anonymous2():
                    ...
                    if not self.__received_pong:
                        self.__dealer_client.logger.warning(
                            "Did not receive pong in 15 seconds. Reconnecting..."
                        )
                        self.close()
                        return
                    self.__received_pong = False

                # Increased timeout from 3 to 15 seconds for better network tolerance
                self.__scheduler.enter(15, 1, anonymous2)
...
class SearchManager:
    """ """
    base_url = "hm://searchview/v6/search/"
    __session: Session
    ...
    def request(self, request: SearchRequest) -> typing.Any:
        ...
        urls = [
            request.build_url_path("hm://searchview/v6/search/"),
            request.build_url_query("hm://searchview/v6/search"),
        ...
        if last_status == 404:
            self.__session.logger.warning(
                "Search endpoint returned 404 (urls tried: {})".format(
                    ", ".join(urls)))
        raise SearchManager.SearchException(last_status or 0)
    ...
    class SearchRequest:
        ...
        def _build_params(self) -> str:
            ...
            params += "&catalogue=" + urllib.parse.quote(self.__catalogue)
            params += "&country=" + urllib.parse.quote(self.__country)
            params += "&imageSize=" + urllib.parse.quote(self.__image_size)
            params += "&limit=" + str(self.__limit)
            params += "&locale=" + urllib.parse.quote(self.__locale)
            params += "&username=" + urllib.parse.quote(self.__username)
            return params

        def build_url_path(self, base_url: str) -> str:
            """ """
            return base_url + urllib.parse.quote(self.query) + "?" + self._build_params()

        def build_url_query(self, base_url: str) -> str:
            """ """
            return base_url + "?query=" + urllib.parse.quote(self.query) + "&" + self._build_params()
