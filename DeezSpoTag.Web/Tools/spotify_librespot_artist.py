...
def main():
    parser = argparse.ArgumentParser(description="Fetch Spotify artist metadata via librespot.")
    parser.add_argument("--credentials", required=True, help="Path to librespot credentials.json")
    parser.add_argument("--artist-id", required=True, help="Spotify artist ID/URI/URL")
    ...
    try:
        payload = client.get_artist(args.artist_id)
    except Exception as exc:
        _write_result(False, error=f"librespot_artist_error: {exc}")
        try:
