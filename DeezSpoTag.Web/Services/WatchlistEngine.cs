...
internal sealed class WatchlistEngine
{
        ...
        bool forceMediaServerSync = false)
    {
        ...
        await _libraryRepository.AddPlaylistWatchTracksAsync(
            ...
            sourceId,
            candidates
                .Select((candidate, index) => new PlaylistWatchTrackInsert(
                    candidate.TrackSourceId,
                    candidate.Isrc,
                    candidate.SourcePosition ?? index + 1,
                    candidate.Title,
                    candidate.Artist,
                    candidate.Album,
        ...
        var visitSyncResult = hasConfiguredPlaylistSyncTargets
            ? await ApplyKnownTargetMembershipInlineAsync(
                currentPlaylist,
                preference,
                candidates,
                candidateIdentityRevision,
        ...
        await UpdatePlaylistStateAsync(
            ...
            liveSnapshot.SnapshotId,
            syncRequestAccepted
                ? hasOutstandingPlaylistWork
                    ? WatchlistPlaylistState.Incomplete
                    : WatchlistPlaylistState.Completed
                : WatchlistPlaylistState.Failed,
            reconciliationMessage,
            nextAttemptUtc: null,
            consecutiveFailures: syncRequestAccepted ? 0 : 1,
            cancellationToken);
        return new PlaylistReconciliationResult(
            syncRequestAccepted,
            reconciliationMessage,
            ...
            FailedTracks: syncRequestAccepted ? 0 : 1,
            SyncResult: visitSyncResult,
            Deferred: false,
            AttemptedTracks: 0,
            QueueStopReason: (hasOutstandingPlaylistWork
                ? WatchQueueStopReason.Incomplete
                : WatchQueueStopReason.Completed).ToString(),
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var (playlist, rows) in groupedRows)
        {
            ...
            var queueOptions = BuildQueueWatchOptions(new QueueWatchOptionsInput
            {
                ...
                ProviderReadinessRevision = rows.FirstOrDefault(row => !string.IsNullOrWhiteSpace(row.ProviderReadinessRevision))?.ProviderReadinessRevision
            });
            var missingTracks = rows
    ...
    private async Task<PlaylistSyncResult?> ApplyKnownTargetMembershipInlineAsync(
        PlaylistWatchlistDto playlist,
        PlaylistWatchPreferenceDto? preference,
        IReadOnlyList<PlaylistTrackCandidate> candidates,
        string? candidateIdentityRevision,
        IReadOnlyList<PlaylistWatchTrackStatusDto> localTrackStatuses,
        bool force,
        CancellationToken cancellationToken)
    {
        ...
        var revision = PlaylistCandidateContract.BuildTargetSyncRevision(
            ...
            localTrackStatuses);
        var playlistSync = _serviceProvider.GetRequiredService<PlaylistSyncService>();
        var requestedTargetSync = false;
        foreach (var target in ResolveConfiguredPlaylistSyncTargets(preference))
        {
            var targetResult = await playlistSync.SyncAvailablePlaylistTracksAsync(
                playlist,
                preference,
                candidates,
                target,
            ...
            if (targetResult.Success)
            {
                var catchUpJobs = await _libraryRepository.EnqueueMembershipJobsForResolvedUnsyncedIdentitiesAsync(
                    ...
                    revision,
                    cancellationToken);
                requestedTargetSync |= catchUpJobs.Count > 0;
            }
            else
            {
                var retryJobs = await _libraryRepository.EnqueueWatchlistPlaylistSyncJobAsync(
                    playlist.Source,
                    playlist.SourceId,
                    revision,
                    target,
                    cancellationToken);
                requestedTargetSync |= retryJobs.Count > 0;
            }

            requestedTargetSync |= await ApplyArtworkInlineForTargetAsync(
                playlistSync,
                playlist,
                preference,
                target,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var artworkResult = await playlistSync.SyncPlaylistArtworkToTargetAsync(
            playlist,
            preference,
            target,
            cancellationToken);
        ...
        if (artworkResult.Success || string.IsNullOrWhiteSpace(revision))
        {
            return false;
        }
        ...
        bool forceArtworkRefresh = false)
    {
        ...
        {
            var candidates = await GetCachedPlaylistTrackCandidatesAsync(source, sourceId, cancellationToken);
            var statuses = await _libraryRepository.GetPlaylistWatchTrackStatusesAsync(source, sourceId, cancellationToken);
            await ApplyKnownTargetMembershipInlineAsync(
                currentPlaylist,
                preference,
                candidates,
                currentPlaylist.SnapshotId,
                statuses,
                force: false,
                cancellationToken);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var orderedCandidates = candidates
            .Select((candidate, index) => new
            {
                Candidate = candidate,
                OriginalIndex = index,
                Position = statusByTrackId.TryGetValue(candidate.TrackSourceId, out var status)
                    ? status.SourcePosition
                    : candidate.SourcePosition
            })
            .OrderBy(static item => item.Position ?? int.MaxValue)
        ...
        foreach (var candidate in orderedCandidates)
        {
            ...
            {
                await TryMarkWatchTrackStatusAsync(source, sourceId, candidate.TrackSourceId, "blocked", cancellationToken);
                ignoredCount++;
                continue;
            }

            if (statusByTrackId.TryGetValue(candidate.TrackSourceId, out var reviewedStatus)
                && string.Equals(reviewedStatus.IdentityStatus, "review", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
            ...
            if (watchTrack != null)
            {
                ...
                var dedupeDecision = await dedupeService.CheckAsync(
                    DownloadDedupeService.FromDownloadIntent(
                        preparedIntent,
                        requestedLocalQualityRank: ResolveWatchRequestedLocalQualityRank(preparedIntent),
    ...
    private async Task<int> ResolveAndPersistLocalTrackIdentitiesAsync(
        string source,
        string sourceId,
        IReadOnlyList<PlaylistTrackCandidate> candidates,
        CancellationToken cancellationToken)
    {
        var inputs = candidates
            .Select(candidate =>
            {
                var identitySource = !string.IsNullOrWhiteSpace(candidate.DeezerId)
                    ? DeezerSource
                    : source;
                var identityTrackId = !string.IsNullOrWhiteSpace(candidate.DeezerId)
                    ? candidate.DeezerId
                    : candidate.TrackSourceId;
        ...
        {
            var identity = await _localIdentityResolver.ResolveAsync(
                inputs[index],
                identities[index],
                cancellationToken);
            ...
            await _libraryRepository.UpdatePlaylistWatchTrackVerificationAsync(
                ...
                new PlaylistWatchTrackVerification(
                    candidates[index].TrackSourceId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch (reasonCode)
        {
            ...
            case "library_duplicate":
                if (!decision.LocalTrackId.HasValue)
                {
            ...
            case "blocklist_match":
                ...
                await TryMarkWatchTrackStatusAsync(source, sourceId, track.TrackId, "blocked", cancellationToken);
    ...
    private static WatchlistHistoryStatus ResolveQueueStopHistoryStatus(WatchQueueStopReason stopReason)
        => stopReason switch
        {
            ...
            WatchQueueStopReason.DownloadGate => WatchlistHistoryStatus.TrackQueueDeferred,
        ...
        IReadOnlyCollection<WatchIntentTrack>? tracks)
    {
        ...
        foreach (var track in tracks)
        {
            ...
            candidates.Add(new PlaylistTrackCandidate(
                ...
                SourceUrl: EmptyToNull(track.Intent.SourceUrl),
                SourcePosition: track.SourcePosition ?? position));
        }
...
private async Task<ApplePlaylistWatchData?> GetApplePlaylistWatchDataAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            sourcePosition++;
            var watchTrack = BuildApplePlaylistWatchTrack(track, storefront, imageUrl, sourcePosition);
        ...
        int sourcePosition)
    {
        ...
        return new WatchIntentTrack(trackId, intent.Isrc, intent, sourcePosition);
    }

    private async Task<string?> GetPersistedAppleStorefrontAsync(
        string playlistId,
        CancellationToken cancellationToken)
    {
        var playlist = await _libraryRepository.GetPlaylistWatchlistEntryAsync(
            AppleSource,
            playlistId,
            cancellationToken);
        return NormalizeAppleStorefront(playlist?.SourceStorefront);
    }
        ...
        IReadOnlyCollection<string> ignoredTrackIds)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            if (statusByTrackId.TryGetValue(candidate.TrackSourceId, out var status))
            {
                ...
                var identityStatus = (status.IdentityStatus ?? string.Empty).Trim().ToLowerInvariant();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (ShouldMarkWatchTrackAsCompleted(result))
        {
            ...
            queuedCount += await TryQueueAtmosIntentAsync(
                ...
                context.Options,
                cancellationToken);
            var libraryDecision = await _serviceProvider.GetRequiredService<DownloadDedupeService>()
                .CheckLibraryPresenceAsync(
                    DownloadDedupeService.FromDownloadIntent(
                        intent,
                        blockRules: context.Options.BlockRules),
                    cancellationToken);
            if (!libraryDecision.LocalTrackId.HasValue)
            {
                await TryMarkWatchTrackStatusAsync(
    ...
    private static int? ResolveWatchRequestedLocalQualityRank(DownloadIntent intent)
        => DownloadDedupeService.ResolveRequestedLocalQualityRank(intent.Quality) ?? 3;
    ...
    private sealed record WatchIntentTrack(
        string TrackId,
        string? Isrc,
        DownloadIntent Intent,
        int? SourcePosition = null);
