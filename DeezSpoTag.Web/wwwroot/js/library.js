...
function buildSpotifyTopTrackPlaybackQueue(startButton) {
    ...
    return Array.from(list.querySelectorAll('.top-song-item__play.track-play'))
        .filter((button) => {
            ...
            return Boolean(deezerId || previewUrl);
        });
}
...
function ensureLibrarySpotifyButtonReadyForPlayback(button, url, options = {}) {
    ...
    const isTopSongButton = isLibraryTopSongPlayButton(button);
    const spotifyUrl = isTopSongButton
        ? ''
        : normalizeSpotifyTrackSourceUrl(String(url || button.dataset.spotifyUrl || '').trim());
    const previewUrl = String(button.dataset.previewUrl || '').trim();
    if (!spotifyUrl) {
...
function renderSpotifyTopTracks(tracks, _artistProfile = null) {
    ...
    list.innerHTML = `
        ...
        const artistName = String(fallbackArtistName || track.artistName || track.artist).trim();
...
function buildArtistPageTrackAvailabilityRequest(track, fallbackId) {
    ...
    const parsedSource = spotifyUrlHelpers.parseSpotifyUrl(track.sourceUrl || '');
    const sourceId = String(track.id || (parsedSource?.type === 'track' ? parsedSource.id : '') || '').trim();
...
function getSpotifyTopTrackMatcher() {
    if (spotifyTopTrackMatcher) {
        return spotifyTopTrackMatcher;
    }

    spotifyTopTrackMatcher = spotifyUrlHelpers.createDeezerSectionMatcher({
        sectionKey: () => `library-top-tracks:${getCurrentLibraryArtistId() || 'unknown'}`,
        normalizeUrl: normalizeSpotifyTrackSourceUrl,
        buildRequest: (button, url) => buildSpotifyTrackResolveRequestFromButton(button) || { link: url },
        fetchJson,
...
async function startSpotifyTopTrackPlaylistStyleMatching(pendingQueue) {
    const entries = Array.isArray(pendingQueue)
        ...
        : [];
    await getSpotifyTopTrackMatcher().start(entries);
}
