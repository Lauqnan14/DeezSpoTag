...
private static readonly JsonSerializerOptions AutoBuildVersionStateWriteOptions = new()
{
    WriteIndented = true
};
...
static void RegisterCoreApplicationServices(IServiceCollection services, IConfiguration configuration)
{
    ...
    services.AddSingleton<DeezSpoTag.Web.Services.LinkMapping.ExternalLinkClassifier>();
    services.AddSingleton<DeezSpoTag.Web.Services.LinkMapping.DeezerLinkParser>();
...
static string ResolveAutoVersionSourceRoot(string contentRootPath)
{
    ...
    var parentCandidate = Path.GetFullPath(Path.Join(normalizedContentRoot, ".."));
    if (IsSafeAutoVersionSourceRoot(parentCandidate))
    {
        return parentCandidate;
    }

    if (IsSafeAutoVersionSourceRoot(normalizedContentRoot))
    {
...
static bool IsSafeAutoVersionSourceRoot(string path)
{
    ...
    if (string.IsNullOrWhiteSpace(normalized) || IsFilesystemRoot(normalized))
    {
        return false;
    }

    return File.Exists(Path.Join(normalized, "src.sln"))
           || Directory.Exists(Path.Join(normalized, ".git"))
           || Directory.Exists(Path.Join(normalized, "DeezSpoTag.Web"));
}
...
static string ResolveAutoBuildVersion(
    string dataRoot,
    string sourceRoot,
    ...
    ILogger logger)
{
    try
    {
        var baseline = CreateBaselineAutoBuildVersionState(assemblyVersion);
        var stateDirectory = Path.Join(dataRoot, "runtime");
        Directory.CreateDirectory(stateDirectory);
        var statePath = Path.Join(stateDirectory, "auto-build-version.json");
        var currentFingerprint = ComputeSourceFingerprint(sourceRoot);
        if (string.IsNullOrWhiteSpace(currentFingerprint))
        {
            return fallbackVersion;
        }

        var loaded = TryReadAutoBuildVersionState(statePath);
        var current = MergeAutoBuildVersionState(loaded, baseline);
        ApplyAutoBuildFingerprintIncrement(loaded, current, currentFingerprint);

        current.Fingerprint = currentFingerprint;
        WriteAutoBuildVersionState(statePath, current);
        return current.ToVersionString();
    }
    catch (Exception ex)
    {
        logger.LogWarning(ex, "Automatic build version bump failed. Falling back to assembly version.");
        return fallbackVersion;
    }
}

static AutoBuildVersionState CreateBaselineAutoBuildVersionState(Version? assemblyVersion)
    => new()
    {
        Major = Math.Max(0, assemblyVersion?.Major ?? 0),
        Minor = Math.Max(0, assemblyVersion?.Minor ?? 0),
        Patch = Math.Max(0, assemblyVersion?.Build ?? 0),
        Revision = Math.Clamp(assemblyVersion?.Revision ?? 0, 0, 9)
    };

static AutoBuildVersionState MergeAutoBuildVersionState(AutoBuildVersionState? loaded, AutoBuildVersionState baseline)
{
    if (loaded == null)
    {
        return baseline;
    ...
    if (loaded.Major != baseline.Major || loaded.Minor != baseline.Minor)
    {
        return baseline;
    }

    if (loaded.Patch < baseline.Patch ||
        (loaded.Patch == baseline.Patch && loaded.Revision < baseline.Revision))
    {
        loaded.Patch = baseline.Patch;
        loaded.Revision = baseline.Revision;
    }

    return loaded;
}

static void ApplyAutoBuildFingerprintIncrement(
    AutoBuildVersionState? loaded,
    AutoBuildVersionState current,
    string currentFingerprint)
{
    var shouldIncrement = loaded != null &&
        !string.Equals(loaded.Fingerprint, currentFingerprint, StringComparison.Ordinal);
    if (!shouldIncrement)
    {
        return;
    }

    if (current.Revision < 9)
    {
        current.Revision++;
        return;
    }
...
static AutoBuildVersionState? TryReadAutoBuildVersionState(string path)
{
    if (!File.Exists(path))
    {
        return null;
    ...
    try
    {
        ...
        var state = JsonSerializer.Deserialize<AutoBuildVersionState>(json);
        if (state == null)
        {
            return null;
        }

        state.Major = Math.Max(0, state.Major);
        state.Minor = Math.Max(0, state.Minor);
        state.Patch = Math.Max(0, state.Patch);
        state.Revision = Math.Clamp(state.Revision, 0, 9);
...
static void WriteAutoBuildVersionState(string path, AutoBuildVersionState state)
{
    var json = JsonSerializer.Serialize(state, AutoBuildVersionStateWriteOptions);
    File.WriteAllText(path, json);
}

static string ComputeSourceFingerprint(string sourceRoot)
{
    var files = CollectFingerprintFiles(sourceRoot);
    if (files.Count == 0)
    {
        return string.Empty;
    }

    return ComputeFingerprintHash(sourceRoot, files);
}

static List<string> CollectFingerprintFiles(string sourceRoot)
{
    ...
    if (!Directory.Exists(normalizedSourceRoot) || IsFilesystemRoot(normalizedSourceRoot))
    {
        return new List<string>();
    }
    ...
    stack.Push(normalizedSourceRoot);
    while (stack.Count > 0)
    {
        var current = stack.Pop();
        if (!TryEnumerateDirectory(current, out var directories, out var currentFiles))
        {
            continue;
        }
        ...
        files.AddRange(currentFiles.Where(file => includeExtensions.Contains(Path.GetExtension(file))));
    }
...
static bool TryEnumerateDirectory(
    string directory,
    out IEnumerable<string> directories,
...
static string ComputeFingerprintHash(string sourceRoot, List<string> files)
{
    ...
    {
        _ = TryAppendFileFingerprint(sha, sourceRoot, file, separator, buffer);
    }
    sha.TransformFinalBlock(Array.Empty<byte>(), 0, 0);
    return Convert.ToHexString(sha.Hash ?? Array.Empty<byte>());
}

static bool TryAppendFileFingerprint(
    ...
    byte[] buffer)
{
    ...
    hashAlgorithm.TransformBlock(header, 0, header.Length, null, 0);
